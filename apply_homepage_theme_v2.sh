...
# 0) Optional: avoid any telemetry writes while building
export ASTRO_TELEMETRY_DISABLED=1

# 1) Strengthen the theme tokens (append once)
mkdir -p src/styles
if ! grep -q '/* --- USPS Theme Enhancements --- */' src/styles/theme.css 2>/dev/null; then
...
.section-hero{ background: var(--ps-blue); color:#fff; padding:56px 0; }
...
# 2) Replace homepage markup (NO touching $HOME!)
INDEX_FILE="src/pages/index.astro"
cp "$INDEX_FILE" "_backup.index.$(date +%s).astro"

cat > "$INDEX_FILE" <<'ASTRO'
---
...
rm -rf dist .astro
npx astro build
echo "✅ Build complete. Use: npx serve -s dist -l 5050"
