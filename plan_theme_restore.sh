...
git diff --name-status "$REF" -- $FILES > reports/theme_restore_plan.tsv || true
git diff "$REF" -- $FILES > reports/theme_restore_plan.patch || true
echo "wrote reports/theme_restore_plan.tsv and reports/theme_restore_plan.patch"
