...
function r(p){try{return fs.readFileSync(p,'utf8')}catch{return''}}
const files = glob.sync('src/pages/**/*.{astro,md,mdx}',{nodir:true});
let missTitle = [], missDesc = [], missH1 = [], missCanon = [];
for (const f of files) {
  if (f.endsWith('_index.stylecheck.astro')) continue;
  const s = r(f);
  const usesSiteLayout = /<SiteLayout\b/.test(s);
  let hasTitle = /<title>/.test(s) || /title=/.test(s);
  let hasDesc = /name="description"/.test(s) || /description=/.test(s);
  let hasH1 = /<h1\b/i.test(s);
  let hasCanon = /rel="canonical"/.test(s) || /canonical=/.test(s);
  if (usesSiteLayout) {
    hasTitle = true;
    hasDesc = true;
    hasCanon = true;
...
function out(k,a){
  if(a.length){
    console.log(k + ': ' + a.length + ' issue(s)');
    console.log(a.slice(0,30).join('\\n'));
  }else{
    console.log(k + ': ok');
  }
}
